#!/bin/bash
#fork from https://github.com/sameersbn/docker-bind/blob/master/entrypoint.sh
set -e

DNS_ROOT_PASSWORD=${DNS_ROOT_PASSWORD:-password}
...
set_root_passwd() {
  echo "root:$DNS_ROOT_PASSWORD" | chpasswd
}
...
named-checkconf
named-checkzone iglu.com /etc/bind/zones/db.iglu.com
named-checkzone 113.10.in-addr.arpa /etc/bind/zones/db.10.113
named-checkzone 100.172.in-addr.arpa /etc/bind/zones/db.172.100
